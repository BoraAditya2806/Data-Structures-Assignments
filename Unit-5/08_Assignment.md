...
#include <climits>
#include <iomanip>
#include <queue> // For optimized implementation (Min-Heap based Prim's)
using namespace std;
...
// Pair for priority queue: (weight, vertex)
...
private:
    ...
    // Find vertex with minimum key value (O(V) linear search)
...
public:
    ...
    // Prim's Minimum Spanning Tree Algorithm (O(V^2) with array/linear search)
    void primsMST_agb() {
        if (vertices_agb == 0) {
        ...
        // key_agb[i] stores the minimum weight to connect vertex i to the MST
        int* key_agb = new int[vertices_agb];
        // parent_agb[i] stores the predecessor of vertex i in the MST
        int* parent_agb = new int[vertices_agb];
        // visited_agb[i] is true if vertex i is already included in MST
        ...
        // Start with vertex 0
        key_agb[0] = 0;      // Make key 0 so that this vertex is picked as first
        parent_agb[0] = -1;  // First node is the root of the MST
        ...
        for (int count_agb = 0; count_agb < vertices_agb; count_agb++) {
            // Step 1: Pick the minimum key vertex from the set of vertices not yet included in MST (O(V) search)
            int u_agb = findMinKey_agb(key_agb, visited_agb);

            if (u_agb == -1) {
                // If the graph is disconnected, we won't be able to find V-1 edges
                if (count_agb < vertices_agb - 1 && vertices_agb > 1) {
                    cout << "Graph is disconnected! Found a spanning forest." << endl;
                }
                break;
            }

            // Step 2: Add the picked vertex to the MST Set
            visited_agb[u_agb] = true;

            cout << "\nStep " << (count_agb + 1) << ": **Adding vertex " << u_agb
                 << "** to MST (Key weight: " << key_agb[u_agb] << ")" << endl;

            // Step 3: Update key value and parent index of the adjacent vertices
            for (const auto& edge_agb : adjList_agb[u_agb]) {
                int v_agb = edge_agb.destination_agb;
                int weight_agb = edge_agb.weight_agb;

                // Check if v is not in MST (visited) and if the weight of (u,v) is smaller than current key of v
                if (!visited_agb[v_agb] && weight_agb < key_agb[v_agb]) {
                    cout << "  Updating vertex " << v_agb << " key: " << key_agb[v_agb]
                         << " -> **" << weight_agb << "** (via " << u_agb << ")" << endl;
                    parent_agb[v_agb] = u_agb;
                    key_agb[v_agb] = weight_agb;
                }
            }
        }
        ...
        // Sum of key values for all vertices except the root (which has key 0)
        for (int i_agb = 1; i_agb < vertices_agb; i_agb++) {
            if (key_agb[i_agb] != INT_MAX) { // Only count connected vertices
    ...
    void displayMST_agb(int parent_agb[], int key_agb[]) {
        ...
        for (int i_agb = 1; i_agb < vertices_agb; i_agb++) {
            if (parent_agb[i_agb] != -1) { // Skip if vertex wasn't reached (disconnected)
    ...
    void displayEdges_agb() {
        ...
        for (int i_agb = 0; i_agb < vertices_agb; i_agb++) {
            for (int j_agb = 0; j_agb < adjList_agb[i_agb].size(); j_agb++) {
                // Only display each edge once (since it's undirected, only print if source < destination)
...
int main_agb() {
    ...
    cout << "===== Prim's Minimum Spanning Tree Algorithm (Adjacency List) =====" << endl;
...
```
===== Prim's Minimum Spanning Tree Algorithm (Adjacency List) =====
...
Vertices: 0, 1, 2, 3
Adjacency List:
Vertex 0: [1,2] [2,4] [3,3]
Vertex 1: [0,2] [2,2] [3,5]
Vertex 2: [0,4] [1,2] [3,6]
Vertex 3: [0,3] [1,5] [2,6]
```
...
1. **Prim's Algorithm with Adjacency List**:

   - Vertex-based greedy algorithm for finding MST
   - Starts from arbitrary vertex and grows MST one vertex at a time
...
6. **Advantages**:
   ...
   - Easy to implement with adjacency list
   - Produces connected, acyclic subgraph

7. **Disadvantages**:

   - Slower for dense graphs compared to adjacency matrix
...
1. **Binary Heap**: For efficient minimum key extraction
2. **Fibonacci Heap**: For better theoretical complexity
3. **Early Termination**: Stop when graph is disconnected
4. **Path Compression**: Optimize parent array access

**Adjacency List vs Adjacency Matrix for Prim's**:
